...
- **Server**: F# with Akka.NET actors for concurrent chat handling, Suave for HTTP/WebSocket serving
...
### Authentication
- Supports OAuth (Google/GitHub) and anonymous users
- OAuth configuration in `CHAT_DATA/suave.oauth.config`
- Session management via Suave cookies
...
- Suave for HTTP/WebSocket handling
- `dotnet watch run` provides hot reload

### Protocol Changes
When modifying `src/Shared/ChatProtocol.fs`:
